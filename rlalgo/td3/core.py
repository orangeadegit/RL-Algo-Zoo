...
class Td3Actor(nn.Module):
    def __init__(self,latent_module,ac_space,indim=256,act_range=(-1.,1.)):
        super().__init__()
        ...
        #self.action_noise = NormalActionNoise(np.zeros(ac_space.shape[-1],0.2*np.ones(ac_space.shape[-1])))
    ...
    def step(self,x,with_logprob=True,deterministic=True):
        dist=self.forward(x)
        if deterministic:
            action=dist.mean
        else:
        ...
        if with_logprob:
            ...
            logp=None
        #action=torch.clamp(torch.tanh(action)+self.action_noise,self.act_range[0],self.act_range[1])
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self,observation_space,action_space,env=None,**ac_kwargs):
        ...
        self.actor=Td3Actor(latent,action_space,)
        latent = Mlp(indim=observation_space.shape[0]+action_space.shape[0],
                     num_hidden=ac_kwargs.get('num_hidden', 256),
                     acti=ac_kwargs.get('activation', nn.ReLU))
        self.q1=Td3Critic(latent,)
        latent = Mlp(indim=observation_space.shape[0] + action_space.shape[0],
                     num_hidden=ac_kwargs.get('num_hidden', 256),
                     acti=ac_kwargs.get('activation', nn.ReLU))
        self.q2 = Td3Critic(latent, )
    ...
    def step(self,obs):
        obs=obs.to(torch.float32)
        return self.actor.step(obs,deterministic=True)